...
We'll start with the Spring PetClinic Sample Application, back as it almost five years ago in 2017!
...
Be sure to commit your changes once every few steps to get a clean diff of changes.
...
Start by installing and activating a recent Java 8 version. 
[source,bash]
----
...
Or add a `.sdkmanrc` file containing `java=8.0.322-tem` inside `spring-petclinic/`, to switch each time you enter the project folder.

=== Apache Maven wrapper
Next up we'll switch out the almost 7 years old Takari Maven Wrapper 3.3.3 for the more recent Apache Maven Wrapper version 3.8.5.
[source,bash]
----
...
== Upgrade to JUnit 5
To gain a bit of confidence with OpenRewrite, we'll first migrate our application from JUnit 4 to 5.
